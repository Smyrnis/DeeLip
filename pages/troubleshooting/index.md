...
`CREATE TABLE IF NOT EXISTS`). Full details: [Health check docs](/docs/install/health-check).
...
DeeLip uses STUN, TURN relay fallback, and full ICE to work around this. See the
[NAT docs](/docs/guide/nat) for how those three compose. One known limitation: STUN
support is IPv4-only today.

## Audio problems (choppy, no audio, echo)

The audio pipeline (capture, echo cancellation, AGC, codecs, RTP/SRTP) is covered in the
[Audio & video quality docs](/docs/guide/audio-video). If calls connect but audio is missing or
distorted, double-check your system's default input/output devices — DeeLip uses whatever ALSA/
PulseAudio reports as default.
...
verification, alongside SDES-SRTP as an alternative. If a call won't establish encryption, see
the ZRTP handshake details in the [Calling & security docs](/docs/guide/calling-security).
