...
impl DeelipApp {
    ...
    /// as `show_transfer_window`, but sourcing `self.calls_state.pending_call` instead of a
    /// connected `self.calls_state.focused_call`, since `do_transfer` can't be reused here
    ...
    pub(crate) fn show_redirect_window(&mut self, ctx: &egui::Context, self_app: SharedApp) {
        show_pop_out_window(
            ...
            [320.0, 600.0],
            false,
            |app| app.calls_state.showing_redirect,
            |app| app.calls_state.showing_redirect = false,
    ...
    fn show_redirect_window_content(&mut self, ui: &mut Ui) {
        ...
        ui.add_space(4.0);
        if let Some(pending) = &self.calls_state.pending_call {
        ...
        ui.separator();
        ui.add_space(8.0);
        transfer_target_editor(ui, palette, &mut self.calls_state.redirect_target);
        ui.add_space(8.0);
