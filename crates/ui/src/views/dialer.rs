...
impl DeelipApp {
    ...
    fn show_dialer_idle(&mut self, ui: &mut Ui) {
        ...
        ui.horizontal(|ui| {
            let call_text = RichText::new(format!("{}  Call", egui_phosphor::regular::PHONE))
                .size(16.0).color(self.palette.accent);
            if ui.add_sized([ui.available_width() * 0.7, 40.0], egui::Button::new(call_text))
                .clicked()
            {
                self.do_call(None);
            }
            let can_redial = self.reg_ok && self.last_dialed.is_some();
            let redial_text = RichText::new(format!("{}  Redial", egui_phosphor::regular::CLOCK_COUNTER_CLOCKWISE)).size(14.0);
    ...
    fn show_incoming_call_screen(&mut self, ui: &mut Ui, from: &str) {
        ...
        ui.vertical_centered(|ui| {
            ui.label(RichText::new(egui_phosphor::regular::PHONE_INCOMING).size(40.0).color(self.palette.info));
            ui.add_space(8.0);
    ...
    fn show_dialing_screen(&mut self, ui: &mut Ui, target: &str) {
        ui.add_space(24.0);
        ui.vertical_centered(|ui| {
            ui.label(RichText::new(egui_phosphor::regular::PHONE_OUTGOING).size(40.0).color(self.palette.accent));
            ui.add_space(8.0);
    ...
    fn show_active_calls(&mut self, ui: &mut Ui) {
        ...
        if let Some(out) = &self.pending_outbound {
            ui.label(RichText::new(format!("Calling {}…", short_uri(&out.remote_uri))).color(self.palette.muted));
            ui.add_space(6.0);
        }
            ...
            theme::full_width_card(ui, self.palette, |ui| {
                if focused {
                    ui.vertical_centered(|ui| {
                        ...
                        let state = if self.in_conference { "In conference" } else { "Active" };
                        ...
                        ui.horizontal(|ui| {
                            ...
                            if !self.in_conference
                                && circular_action_button(ui, egui_phosphor::regular::PHONE_PAUSE, self.palette.muted)
                            {
                                hold_idx = Some(idx);
                            }
                        });
