...
impl DeelipApp {
    ...
    pub(super) fn show_directory_section(&mut self, ui: &mut Ui, palette: &Palette) -> bool {
        ...
        theme::full_width_card(ui, *palette, |ui| {
            egui::Grid::new("settings_ldap_grid").num_columns(2).spacing([8.0, 4.0]).show(ui, |ui| {
                field_label(ui, palette, &t("settings.account.server_label"));
                edited |= optional_text_field(
                    ui,
                    palette,
                    &mut self.config.ldap_server,
                    &t("settings.directory.server_hint"),
                );
                ui.end_row();
                ...
                edited |= optional_text_field(
                    ui,
                    palette,
                    &mut self.config.ldap_base_dn,
                    &t("settings.directory.base_dn_hint"),
                );
                ui.end_row();
                ...
                ui.horizontal(|ui| {
                    edited |= optional_text_field_sized(
                        ui,
                        palette,
                        &mut self.config.ldap_bind_dn,
                        &t("settings.directory.bind_dn_hint"),
                        240.0,
                    );
            ...
            ui.horizontal(|ui| {
                edited |=
                    ui.checkbox(&mut self.config.ldap_use_tls, t("settings.directory.use_tls_checkbox")).changed();
            ...
            ui.horizontal(|ui| {
                edited |= optional_text_field_sized(
                    ui,
                    palette,
                    &mut self.config.ldap_search_filter,
                    "(|(cn=*{query}*)(mail=*{query}*))",
                    240.0,
                );
