...
impl DeelipApp {
    ...
    pub(super) fn show_notifications_section(&mut self, ui: &mut Ui, palette: &Palette) {
        settings_section(ui, palette, &t("settings.notifications_section_title"), Some(&t("settings.applies_immediately_hint")), |ui| {
            if ui.checkbox(&mut self.config.notifications_enabled, t("settings.desktop_notification_checkbox")).changed() {
            ...
            ui.horizontal(|ui| {
                if ui.checkbox(&mut self.config.random_popup_position, t("settings.random_popup_checkbox")).changed() {
                    self.save_config_quietly();
                }
    ...
    pub(super) fn show_call_handling_section(&mut self, ui: &mut Ui, palette: &Palette) {
        settings_section(ui, palette, &t("settings.call_handling_section_title"), Some(&t("settings.applies_immediately_hint")), |ui| {
            ui.horizontal(|ui| {
    ...
    pub(super) fn show_startup_section(&mut self, ui: &mut Ui, palette: &Palette) -> bool {
        ...
        settings_section(ui, palette, &t("settings.startup_section_title"), None, |ui| {
            ui.horizontal(|ui| {
                edited |= ui.checkbox(&mut self.config.start_minimized, t("settings.start_minimized_checkbox")).changed();
            ...
            ui.add_space(4.0);
            ui.horizontal(|ui| {
                edited |= ui.checkbox(&mut self.config.crash_reporting_enabled, t("settings.crash_reporting_checkbox")).changed();
