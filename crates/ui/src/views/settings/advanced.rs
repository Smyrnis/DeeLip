...
impl DeelipApp {
    ...
    pub(super) fn show_updates_section(&mut self, ui: &mut Ui, palette: &Palette) {
        settings_section(ui, palette, &t("settings.advanced.updates_section_title"), Some(&t("settings.applies_immediately_hint")), |ui| {
            ui.label(RichText::new(tf("settings.advanced.version_label", &[("version", env!("CARGO_PKG_VERSION"))])).color(palette.ink_muted));
            ui.add_space(4.0);
            ui.horizontal(|ui| {
            ...
            ui.add_space(4.0);
            ui.horizontal(|ui| {
                if ui.checkbox(&mut self.config.auto_update_enabled, t("settings.advanced.auto_update_checkbox")).changed() {
                    self.save_config_quietly();
                }
            ...
            ui.horizontal(|ui| {
                ...
                let status = match &self.update_state {
                    ...
                    crate::update::UpdateState::Available(r) => tf("settings.advanced.update_available", &[("version", &r.version)]),
                    crate::update::UpdateState::Downloading => t("settings.advanced.update_downloading"),
                    crate::update::UpdateState::Updated(v)  => tf("settings.advanced.update_updated", &[("version", v)]),
                    crate::update::UpdateState::Failed(e)   => tf("settings.advanced.update_failed", &[("error", &e.to_string())]),
                };
    ...
    pub(super) fn show_blocklist_section(&mut self, ui: &mut Ui, palette: &Palette) {
        settings_section(ui, palette, &t("settings.advanced.blocklist_section_title"), Some(&t("settings.applies_immediately_hint")), |ui| {
            ui.horizontal(|ui| {
                text_edit_scope(ui, palette, |ui| ui.add(egui::TextEdit::singleline(&mut self.blocklist_input)
                    .hint_text(RichText::new(t("settings.advanced.blocklist_hint")).color(palette.ink_muted))
                    .desired_width(200.0)));
