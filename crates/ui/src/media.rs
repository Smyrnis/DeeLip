...
impl DeelipApp {
    ...
    pub(crate) fn start_media(&mut self, idx: usize) {
        ...
        let engine = rt.block_on(MediaEngine::start(
            media.local_rtp, media.remote_rtp, media.codec, media.dtmf_type, media.srtp, media.relay,
            self.config.audio.echo_cancellation,
            input_device.as_deref(), output_device.as_deref(),
            self.config.recording_enabled, &call_id,
            None,
        ));
        match engine {
    ...
    pub(crate) fn start_conference(&mut self) {
        ...
        let engine = rt.block_on(MediaEngine::start(
            media0.local_rtp, media0.remote_rtp, media0.codec, media0.dtmf_type, media0.srtp, media0.relay,
            self.config.audio.echo_cancellation,
            input_device.as_deref(), output_device.as_deref(),
            self.config.recording_enabled, &call_id0,
            Some(leg2),
        ));
        match engine {
            Ok(e) => {
