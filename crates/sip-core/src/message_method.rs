    ...
    pub(crate) async fn send_message(&mut self, to: &str, body: &str) {
        ...
        if let Err(e) = self.transport.send(msg.as_bytes(), self.server_addr).await {
            ...
            let _ = self.event_tx.send(SipEvent::MessageSendResult {
                to: to.to_string(), ok: false, reason: Some(e.to_string()),
            });
            return;
        }
        self.pending_messages.insert(call_id, PendingMessage {
            to: to.to_string(), body: body.to_string(), from_tag, auth_retried: false,
        });
    }
        ...
        match status {
            200..=299 => {
                if let Some(pending) = self.pending_messages.remove(&call_id) {
                    let _ = self.event_tx.send(SipEvent::MessageSendResult {
                        to: pending.to, ok: true, reason: None,
                    });
                }
            }
            401 | 407 => {
                ...
                let Some((to, body, from_tag, auth_retried)) = self.pending_messages.get(&call_id)
                ...
                if auth_retried {
                    ...
                    let _ = self.event_tx.send(SipEvent::MessageSendResult {
                        to, ok: false, reason: Some(format!("{status}")),
                    });
                    return;
                }
                ...
                let Some(challenge_raw) = msg.header(hdr_name) else {
                    self.pending_messages.remove(&call_id);
                    let _ = self.event_tx.send(SipEvent::MessageSendResult {
                        to, ok: false, reason: Some("Missing auth challenge".into()),
                    });
                    return;
                };
                let Some(auth) = build_challenge_response(
                    &self.account.username, &self.account.password, "MESSAGE", &to, challenge_raw,
                ) else {
                    self.pending_messages.remove(&call_id);
                    let _ = self.event_tx.send(SipEvent::MessageSendResult {
                        to, ok: false, reason: Some("Bad auth challenge".into()),
                    });
                    return;
                };
            ...
            c if c >= 300 => {
                if let Some(pending) = self.pending_messages.remove(&call_id) {
                    let _ = self.event_tx.send(SipEvent::MessageSendResult {
                        to: pending.to, ok: false, reason: Some(format!("{c}")),
                    });
                }
            }
            _ => {}
