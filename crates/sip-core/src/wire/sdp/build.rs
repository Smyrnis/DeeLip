...
/// `dtls` takes precedence over `srtp` -- the two are mutually exclusive by
/// construction (an account's `MediaEncryption` selects at most one of
/// SDES/DTLS-SRTP), but `dtls`'s presence is checked first in case a caller
/// ever passes both by mistake.
pub(super) fn savp_profile(srtp: Option<&SrtpParams>, dtls: Option<&DtlsFingerprint>) -> &'static str {
    if dtls.is_some() {
        "UDP/TLS/RTP/SAVP"
    } else if srtp.is_some() {
        "RTP/SAVP"
    } else {
        "RTP/AVP"
    }
...
/// Emits `a=fingerprint`/`a=setup` for DTLS-SRTP (RFC 5763/8122). Both must
/// be present together -- `setup` is meaningless without a fingerprint to
/// authenticate, and vice versa.
pub(super) fn fingerprint_setup_lines(fp: Option<&DtlsFingerprint>, setup: Option<Setup>) -> String {
    let (Some(fp), Some(setup)) = (fp, setup) else { return String::new() };
    format!("{}a=setup:{}\r\n", fp.to_line(), setup.as_str())
}
...
pub fn build_offer(
    ...
    vad_enabled: bool, dtls_fp: Option<&DtlsFingerprint>, dtls_setup: Option<Setup>,
) -> String {
    let sid = now_ntp();
...
/// Build a hold SDP (a=sendonly) for re-INVITE. `dtls_fp`/`dtls_setup`, if
/// the call already negotiated DTLS-SRTP, just repeat the values already
/// established at call setup -- hold/resume never renegotiate the DTLS
/// session.
pub fn build_hold_offer(
    local_ip: &str, rtp_port: u16, codec: AudioCodec, srtp: Option<&SrtpParams>, dtls_fp: Option<&DtlsFingerprint>,
    dtls_setup: Option<Setup>,
) -> String {
    let sid = now_ntp();
    let pt = codec.payload_type();
    let profile = savp_profile(srtp, dtls_fp);
...
/// Build a resume SDP (a=sendrecv) for re-INVITE. See `build_hold_offer`'s
/// doc comment re: `dtls_fp`/`dtls_setup` never being renegotiated here.
